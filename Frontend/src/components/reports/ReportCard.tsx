...
const ReportCard: React.FC<{
  ...
  return (
  <article id={`report-${report.id}`} className={`${selected ? 'bg-blue-50 border-blue-300 ring-1 ring-blue-200' : 'bg-white'} rounded-xl shadow-sm relative overflow-hidden border ${selected ? 'border-blue-300' : 'border-gray-200'} transition-shadow hover:shadow-md`}>
  ...
  <div className={`mt-4 sm:mt-0 sm:w-40 flex-shrink-0 text-left sm:text-right sm:ml-4 ${locked && userRole === 'admin' ? 'sm:flex sm:items-center sm:justify-end' : ''}`}>
              ...
              {onChangeStatus ? (
                  ...
                  // locked and not admin: show a read-only status badge on the right for staff
