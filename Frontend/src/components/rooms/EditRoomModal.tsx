...
const EditRoomModal: React.FC<Props> = ({ room, onClose, onUpdate }) => {
  const [form, setForm] = useState({
    roomNumber: room.name || '',
    roomType: room.type ? room.type.toLowerCase() : 'single',
    capacity: room.capacity || 1,
    status: room.status || 'available',
    monthlyRent: room.rent ? room.rent.replace(/[^\d.]/g, '') : '',
    securityDeposit: room.securityDeposit ? room.securityDeposit.replace(/[^\d.]/g, '') : '',
  ...
  const toggleAmenity = (amenity: string) => {
    setForm(prev => ({
      ...prev,
      amenities: prev.amenities.includes(amenity) ? prev.amenities.filter((a: string) => a !== amenity) : [...prev.amenities, amenity]
    }));
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div ref={modalRef} className="relative w-full max-w-3xl bg-white rounded-lg shadow-lg overflow-hidden flex flex-col max-h-[90vh]">
        <div className="flex items-start justify-between p-4 border-b flex-shrink-0">
          <div>
            <h3 className="text-lg font-semibold">Edit Room</h3>
            <p className="text-sm text-gray-500">Update room information for {room.name}</p>
          </div>
          ...
          <section className="mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input value={form.roomNumber} onChange={e => handleChange('roomNumber', e.target.value)} className="w-full mt-1 px-3 py-2 border rounded-lg text-sm" aria-invalid={!!errors.roomNumber} aria-describedby={errors.roomNumber ? 'edit-roomNumber-error' : undefined} />
                {errors.roomNumber && <p id="edit-roomNumber-error" className="text-xs text-red-500 mt-1">{errors.roomNumber}</p>}
              </div>
          ...
          <section className="mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input value={form.monthlyRent} onChange={e => handleChange('monthlyRent', e.target.value)} className="w-full mt-1 px-3 py-2 border rounded-lg text-sm" aria-invalid={!!errors.monthlyRent} aria-describedby={errors.monthlyRent ? 'edit-monthlyRent-error' : undefined} />
                {errors.monthlyRent && <p id="edit-monthlyRent-error" className="text-xs text-red-500 mt-1">{errors.monthlyRent}</p>}
              </div>

              <div>
                <label className="block text-xs text-gray-600">Security Deposit</label>
                <input value={form.securityDeposit} onChange={e => handleChange('securityDeposit', e.target.value)} className="w-full mt-1 px-3 py-2 border rounded-lg text-sm" aria-invalid={!!errors.securityDeposit} aria-describedby={errors.securityDeposit ? 'edit-securityDeposit-error' : undefined} />
              </div>
            </div>
          </section>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {amenitiesList.map(a => (
                <button
                  ...
                  className={`text-sm px-3 py-2 border rounded-lg text-left ${form.amenities.includes(a) ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200'}`}>
                  {a}
                </button>
              ))}
            </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input value={form.floor} onChange={e => handleChange('floor', e.target.value)} className="w-full mt-1 px-3 py-2 border rounded-lg text-sm" aria-invalid={!!errors.floor} aria-describedby={errors.floor ? 'edit-floor-error' : undefined} />
              </div>
              <div>
                <label className="block text-xs text-gray-600">Area (sq. meters)</label>
                <input value={form.area} onChange={e => handleChange('area', e.target.value)} className="w-full mt-1 px-3 py-2 border rounded-lg text-sm" aria-invalid={!!errors.area} aria-describedby={errors.area ? 'edit-area-error' : undefined} />
              </div>
            </div>
          </section>
        </div>
        ...
        <div className="p-4 border-t bg-white flex-shrink-0 flex items-center justify-end gap-3">
          ...
          <button onClick={handleSubmit} className="px-4 py-2 bg-blue-600 text-white rounded-md">Update Room</button>
        </div>
      </div>
    </div>
  );
